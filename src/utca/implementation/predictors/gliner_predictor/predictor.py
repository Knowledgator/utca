...
from utca.implementation.predictors.transformers_predictor.utils import ensure_dict
...
class GLiNERPredictor(
    ...
    def __init__(
        ...
        self.model = GLiNER.from_pretrained(cfg.model_name).to(cfg.device)
        super().__init__(
    ...
    def invoke(self, input_data: Input, evaluator: Evaluator) -> Dict[str, Any]:
        ...
        outputs = self.model.batch_predict_entities(**inputs)
        return ensure_dict(outputs) # type: ignore
