...
from transformers.utils import ( # type: ignore
    ExplicitEnum,
    add_end_docstrings, # type: ignore
    is_tf_available,
    is_torch_available,
)
from transformers.pipelines.base import ArgumentHandler, ChunkPipeline, Dataset, build_pipeline_init_args # type: ignore


if is_tf_available():
    import tensorflow as tf # type: ignore
    from transformers.models.auto.modeling_tf_auto import TF_MODEL_FOR_TOKEN_CLASSIFICATION_MAPPING_NAMES # type: ignore
if is_torch_available():
    from transformers.models.auto.modeling_auto import MODEL_FOR_TOKEN_CLASSIFICATION_MAPPING_NAMES # type: ignore
...
class TokenClassificationArgumentHandler(ArgumentHandler):
    ...
    def __call__(self, inputs: Union[str, List[str]], **kwargs: Any):
...
class TokenClassificationPipeline(ChunkPipeline):
    ...
    def __init__(self, args_parser=TokenClassificationArgumentHandler(), *args: Any, **kwargs: Any):
    ...
    def __call__(self, inputs: Union[str, List[str]], **kwargs: Any):
        ...
        _, offset_mapping = self._args_parser(inputs, **kwargs)
        if offset_mapping:
    ...
    def postprocess(self, all_outputs, aggregation_strategy=AggregationStrategy.NONE, ignore_labels=None):
        ...
        for model_outputs in all_outputs:
            ...
            word_ids = all_outputs[0]["word_ids"]
            ...
            pre_entities = self.gather_pre_entities(
                sentence, input_ids, scores, word_ids, offset_mapping, special_tokens_mask, aggregation_strategy
            )
    ...
    def aggregate(self, pre_entities: List[Dict[str, Any]], aggregation_strategy: AggregationStrategy) -> List[Dict[str, Any]]:
    ...
    def aggregate_word(self, entities: List[Dict[str, Any]], aggregation_strategy: AggregationStrategy) -> Dict[str, Any]:
    ...
    def aggregate_words(self, entities: List[Dict[str, Any]], aggregation_strategy: AggregationStrategy) -> List[Dict[str, Any]]:
        """
        ...
        for entity in entities:
            ...
            if word_group is None:
                word_group = [entity]
            elif word_id is not None and word_id==prev_word_id:
                word_group.append(entity)
            elif word_id is None and entity["is_subword"]:
                word_group.append(entity)
            else:
