...
class QdrantCreateCollection(QdrantAction[Dict[str, Any], bool]):
    ...
    Args:
        ...
        shard_number: Number of shards in collection. Default is 1, minimum is 1. sharding_method:
            Defines strategy for shard creation. Option auto (default) creates defined number of shards automatically. Data will be distributed between shards automatically. After creation, shards could be additionally replicated, but new shards could not be created. Option custom allows to create shards manually, each shard should be created with assigned unique shard_key. Data will be distributed between based on shard_key value.
        ...
        hnsw_config: Params for HNSW index optimizers_config: Params for optimizer wal_config: Params for Write-Ahead-Log quantization_config: Params for quantization, if None - quantization will be disabled init_from: Use data stored in another collection to initialize this collection timeout:
...
class QdrantDeleteCollection(QdrantAction[Dict[str, Any], bool]):
    ...
    Args:
        collection_name: Name of the collection to delete timeout:
            Wait for operation commit timeout in seconds. If timeout is reached - request will return with service error.
...
class QdrantUpsert(QdrantAction[Dict[str, Any], Dict[str, Any]]):
    """
...
class QdrantDelete(QdrantAction[Dict[str, Any], Dict[str, Any]]):
    def execute(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        return {
...
class QdrantQuery(QdrantAction[Dict[str, Any], List[Any]]):
    """
    ...
    Args:
        ...
        query_filter: - Exclude vectors which doesn't fit given conditions. 
    ...
    Returns:
        List[Any]: List of scored points.
    """
...
class QdrantSearch(QdrantAction[Dict[str, Any], List[Any]]):
    """
